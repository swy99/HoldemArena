...
async def register_party(party: List[str]):
    ...
    # ✅ 파티 인원이 MATCH_SIZE보다 크면 바로 시작
    if len(party) > MATCH_SIZE:
        print(f"[Matchmaker] 단독 파티로 게임 시작: {party}")
        group_id = str(uuid.uuid4())

        player_ids = [get_username_by_uid(uid) for uid in party]
        await request_initialize_quick_play_game(
            game_id=group_id,
            uids=party,
...
async def request_initialize_quick_play_game(game_id: str, uids: list[str], player_ids: list[str]):
    ...
    queue_name = f"game_registry_manager_queue_{worker_index}"
