...
class RoundManager:
    def __init__(self, chips: list[int], sb=1, bb=2, base_time=15.0, timebank=30.0, grace=1.0):
        ...
        self.start_times = [0.0 for _ in range(self.n)]
        self.deadlines = [0.0 for _ in range(self.n)]
        self.grace = grace
    ...
    def start_turn(self, i: int):
        ...
        self.deadlines[i] = now + self.base_time + self.timebanks[i]
        print(f"[RoundManager] start_turn({i}) → deadline set to {self.deadlines[i]:.2f} (now={now:.2f}, timebank={self.timebanks[i]:.2f})")

    def use_timebank(self, i: int):
        elapsed = time.time() - self.start_times[i]
